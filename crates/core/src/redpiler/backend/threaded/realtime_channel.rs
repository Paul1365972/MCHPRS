...
// Make sure two inners do not share a cache line
// TODO: Replace with cache padded from crossbeam-utils
#[repr(align(64))]
struct Inner<T> {
    data_ptr: *mut T,
    capacity: usize,
...
impl<T> Sender<T> {
    ...
    pub unsafe fn begin_commit(&mut self, reserve: usize) {
        let capacity = self.buffer.write.capacity;
        let write_idx = self.buffer.write.idx.load(Ordering::Relaxed);
    ...
    pub unsafe fn send_unsafe(&mut self, value: T) {
        let capacity = self.buffer.write.capacity;
        let write_idx = self.buffer.write.idx.load(Ordering::Relaxed);
    ...
    pub unsafe fn end_commit(&mut self) {
        let write_idx = self.buffer.write.idx.load(Ordering::Relaxed);
...
impl<T> Receiver<T> {
    ...
    pub unsafe fn recv_unsafe(&mut self) -> T {
        let capacity = self.buffer.read.capacity;
        let read_idx = self.buffer.read.idx.load(Ordering::Relaxed);
