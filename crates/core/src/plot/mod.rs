...
use scheduling::{batch_duration, RateSchedule, PLAYER_UPDATE_INTERVAL};
...
impl Plot {
    ...
    fn restart_tick_schedule(&mut self) {
        let now = Instant::now();
        self.tick_schedule = RateSchedule::new(now);
    ...
    fn update(&mut self) {
        ...
        if !self.players.is_empty() {
            ...
            let deadline = now + batch_duration(self.world.output.rate().0);
            let ticks_due = self.tick_schedule.ticks_due(now, self.tps);
    ...
    fn from_data(
        ...
        Plot {
            ...
            tick_schedule: RateSchedule::new(Instant::now()),
            message_receiver: rx,
            message_sender: tx,
