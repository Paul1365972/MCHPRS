...
//! Every node is seeded with its current output strength, then a worklist grows each set by what
//! the sets of its inputs allow until nothing changes. Sets only grow, so the worklist terminates
//! and can retain tighter bounds through cycles. Correlations between inputs are not tracked.
//!
...
impl<W: World> Pass<W> for SSSetAnalysis {
    fn run_pass(
        ...
        for node_idx in graph.node_indices() {
            ...
            queued[node_idx.index()] = true;
            worklist.push(node_idx);
        }

        while let Some(node_idx) = worklist.pop() {
            queued[node_idx.index()] = false;
...
fn evaluate(ty: &NodeType, state: &NodeState, default_input: SSSet, side_input: SSSet) -> SSSet {
    match ty {
        NodeType::Repeater { .. } => {
            if stays_locked(state, side_input) {
...
fn stays_locked(state: &NodeState, side_input: SSSet) -> bool {
    state.repeater_locked && !side_input.contains(0)
}
